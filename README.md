...
- **Script Elements**<br>(larger) scripts may be provided as a `<script type="rsc-script"/>` element within the component it belongs to - e.g., below all other inner elements. This approach keeps the internal structure of an RSC component visible and still allows code to be kept close to the component it belongs to
